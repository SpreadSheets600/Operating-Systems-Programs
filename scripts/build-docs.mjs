...
async function walk(directory, relative = "") {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      if (!entry.name.startsWith(".") && !ignoredDirectories.has(entry.name)) files.push(...await walk(path.join(directory, entry.name), relativePath));
...
function outputPathFor(sourcePath) {
  return sourcePath === "README.md" ? "index.html" : sourcePath.replace(/\.md$/i, ".html");
}
...
function addClass(token, className) {
  token.attrSet("class", [token.attrGet("class"), className].filter(Boolean).join(" "));
}
...
function titleFrom(source, sourcePath) {
  return source.match(/^#\s+(.+)$/m)?.[1]?.trim() ?? path.posix.basename(sourcePath, ".md");
}

function isShellLanguage(language) {
  return ["bash", "shell", "sh", "zsh", "console", "terminal"].includes(language.toLowerCase());
}
...
function makeMarkdown(documentPaths) {
  ...
  rules.heading_open = (tokens, index, options, env, self) => {
    ...
    if (token.tag !== "h1") env.headings.push({ id, text: text.replace(/[`*_]/g, ""), level: Number(token.tag.slice(1)) });
  ...
  rules.bullet_list_open = (tokens, index, options, env, self) => {
    addClass(tokens[index], "my-5 list-disc space-y-2 ps-6 leading-7 text-base-content/80");
    return self.renderToken(tokens, index, options);
  };
  rules.ordered_list_open = (tokens, index, options, env, self) => {
    addClass(tokens[index], "my-5 list-decimal space-y-2 ps-6 leading-7 text-base-content/80");
    return self.renderToken(tokens, index, options);
  };
  ...
  rules.blockquote_open = (tokens, index, options, env, self) => {
    addClass(tokens[index], "my-7 border-s-2 border-primary bg-base-200 px-5 py-1 text-base-content/80");
    return self.renderToken(tokens, index, options);
  };
  rules.link_open = (tokens, index, options, env, self) => {
    ...
    if (linkPath && !/^(?:[a-z][a-z\d+.-]*:|\/|#)/i.test(linkPath)) {
      const target = path.posix.normalize(path.posix.join(path.posix.dirname(env.sourcePath), linkPath));
      if (documentPaths.has(target)) token.attrSet("href", `${hrefFrom(env.outputPath, outputPathFor(target))}${suffix}`);
    }
  ...
  rules.image = (tokens, index, options, env, self) => {
    ...
    addClass(tokens[index], "my-7 h-auto max-w-full rounded-box border border-base-300");
    return self.renderToken(tokens, index, options);
  };
  ...
  rules.code_inline = (tokens, index) => `<code class="rounded-field bg-base-200 px-1.5 py-0.5 font-mono text-[0.85em] text-base-content">${escapeHtml(tokens[index].content)}</code>`;
  rules.fence = (tokens, index) => {
    ...
    const lines = tokens[index].content.replace(/\r?\n$/, "").split(/\r?\n/);
    const renderedLines = lines.map((line) => `<pre${prefix}><code class="language-${escapeHtml(language)}">${escapeHtml(line || " ")}</code></pre>`).join("");
    return `<div class="mockup-code my-7 overflow-x-auto border border-base-300 text-sm shadow-none">${renderedLines}</div>`;
  };
  rules.table_open = () => '<div class="my-7 overflow-x-auto rounded-box border border-base-300"><table class="table table-zebra">';
...
function makeTree(documents, assets) {
  ...
  documents.forEach((document) => insert(document.sourcePath, document, "document"));
  assets.forEach((asset) => insert(asset, asset, "asset"));
  return rootNode;
}

function renderTree(node, activePath, prefix = "") {
  const folders = [...node.folders.entries()].sort(([a], [b]) => a.localeCompare(b, undefined, { numeric: true }));
  const files = [...node.files].sort((a, b) => (a.name === "README.md" ? -1 : b.name === "README.md" ? 1 : a.name.localeCompare(b.name, undefined, { numeric: true })));
  const renderedFolders = folders.map(([name, child]) => {
  ...
  const renderedFiles = files.map(({ name, item, type }) => {
...
function tableOfContents(headings, className = "rounded-box border border-base-300 bg-base-100 p-4") {
...
const clientScript = `(() => {
  ...
  const systemTheme = window.matchMedia("(prefers-color-scheme: dark)").matches ? "lab-night" : "lab";
  const setTheme = (theme) => {
    document.documentElement.dataset.theme = theme;
    if (themeToggle) themeToggle.checked = theme === "lab-night";
    try { localStorage.setItem(key, theme); } catch {}
  };
  let savedTheme;
  try { savedTheme = localStorage.getItem(key); } catch {}
  setTheme(savedTheme || systemTheme);
  themeToggle?.addEventListener("change", () => setTheme(themeToggle.checked ? "lab-night" : "lab"));
  if (drawer) {
    let savedDrawer;
...
function pageTemplate({ title, body, activePath, documents, assets, siteTitle }) {
...
<head>
  ...
  <script>try{document.documentElement.dataset.theme=localStorage.getItem("markdown-site-theme")||(matchMedia("(prefers-color-scheme: dark)").matches?"lab-night":"lab")}catch(e){}</script>
  ...
  <div class="drawer">
    ...
    <div class="drawer-content flex min-h-screen flex-col">
      <header class="navbar sticky top-0 z-30 min-h-15 border-b border-base-300 bg-base-100/95 px-4 backdrop-blur sm:px-8">
        ...
        <div class="navbar-end"><label class="swap swap-rotate btn btn-circle btn-ghost btn-sm" title="Toggle theme"><input data-theme-toggle type="checkbox" value="lab-night" class="theme-controller" aria-label="Toggle dark theme">${icon("sun", "swap-off size-4")} ${icon("moon", "swap-on size-4")}</label></div>
      </header>
      ${body}
    </div>
...
function documentBody(document, assets) {
  const relatedAssets = assets.filter((asset) => path.posix.dirname(asset) === path.posix.dirname(document.sourcePath));
  const attachments = relatedAssets.length ? `<section class="mt-14 border-t border-base-300 pt-8"><p class="font-mono text-xs uppercase tracking-wider text-base-content/55">Files beside this note</p><ul class="list mt-3">${relatedAssets.map((asset) => `<li class="list-row rounded-box border border-base-300"><span class="flex size-9 items-center justify-center rounded-field bg-base-200 text-primary">${icon("file")}</span><a class="list-col-grow link link-hover font-medium" href="${escapeHtml(hrefFrom(document.outputPath, asset))}">${escapeHtml(path.posix.basename(asset))}</a>${icon("arrow", "size-4 text-base-content/50")}</li>`).join("")}</ul></section>` : "";
  const compactToc = document.headings.length ? `<div class="collapse collapse-arrow mb-8 border border-base-300 bg-base-100 xl:hidden"><input type="checkbox"><div class="collapse-title min-h-0 py-3 text-sm font-medium">On this page</div><div class="collapse-content">${tableOfContents(document.headings, "p-0")}</div></div>` : "";
  const trail = document.sourcePath === "README.md" ? "README.md" : document.sourcePath;
...
const assets = allFiles.filter((file) => !/\.md$/i.test(file)).sort((a, b) => a.localeCompare(b, undefined, { numeric: true }));
...
for (const sourcePath of markdownFiles) {
  ...
  const rendered = markdown.render(source, env).replace(/^\s*<h1[^>]*>[\s\S]*?<\/h1>\s*/, "");
  documents.push({ sourcePath, outputPath, title: titleFrom(source, sourcePath), rendered, headings: env.headings });
}

const homeDocument = documents.find((document) => document.sourcePath === "README.md");
if (!homeDocument) throw new Error("Add a root README.md. It is rendered as the site homepage and your repository README.");
const siteTitle = homeDocument.title;

for (const document of documents) {
  const destination = path.join(outputDir, document.outputPath);
  await fs.mkdir(path.dirname(destination), { recursive: true });
  await fs.writeFile(destination, pageTemplate({
...
console.log(`Built ${documents.length} Markdown document(s) and ${assets.length} asset(s) into ${path.relative(root, outputDir)}/`);
